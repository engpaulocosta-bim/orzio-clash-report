...
{
    /// <summary>Validates that no JSON object in the run-index document repeats a property name at any depth.</summary>
    internal static class DuplicatePropertyValidator
    {
        ...
        private static void CheckObject(JsonElement objectElement, string path)
        {
            ...
            foreach (JsonProperty property in objectElement.EnumerateObject())
            {
                ...
                {
                    throw new RunIndexFormatException($"Duplicate JSON property '{property.Name}' at {path}.");
                }
            }
