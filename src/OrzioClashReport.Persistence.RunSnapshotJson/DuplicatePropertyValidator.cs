...
{
    /// <summary>
    /// Validates that no JSON object in the snapshot document repeats a property name, at any depth. A run
    /// snapshot is immutable evidence, so a document must fail instead of silently depending on the
    /// deserializer's last-value-wins behavior for duplicate keys. Property names are compared with
    /// <see cref="StringComparer.Ordinal"/>, matching the contract's case-sensitive property names. This is an
    /// independent, adapter-local copy: the persistence adapter never depends on the run manifest adapter.
    /// </summary>
