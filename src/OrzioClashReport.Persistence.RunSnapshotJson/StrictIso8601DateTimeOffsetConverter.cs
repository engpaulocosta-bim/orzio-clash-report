...
namespace OrzioClashReport.Persistence.RunSnapshotJson
{
    ...
    /// as UTC; that is exactly the silent inference this snapshot format must never perform. On write it
    /// canonicalizes to a seven-fraction-digit ISO 8601 form, preserving the represented offset (never
    /// converting to local time or to UTC). This is an independent, adapter-local copy: the persistence
    /// adapter never depends on the run manifest adapter.
    /// </summary>
